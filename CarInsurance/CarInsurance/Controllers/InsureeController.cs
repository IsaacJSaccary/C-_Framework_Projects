...
namespace CarInsurance.Controllers
{
    public class InsureeController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "Id,FirstName,LastName,EmaiLAddress,DateOfBirth,CarYear,CarMake,CarModel,DUI,SpeedingTicket,CoverageType,Quote")] Table table)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (table.CarMake == "Porsche" && table.CarModel == "Carrera")
                {
                    table.Quote += 25;
                }

                if (table.SpeedingTicket > 0)
                {
                    table.Quote += (table.SpeedingTicket * 10);
                }

                if (table.DUI == true)
                {
                    decimal total = (decimal).25;
                    decimal mulitplier = total * table.Quote;
                    table.Quote += mulitplier;
                }
